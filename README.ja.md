...
- **ドラッグ＆ドロップ**: 外部アプリへのネイティブドラッグ、外部からのドロップおよびペイン間のドラッグは移動（Ctrl キーでコピー、macOS は Option キー）。**Wayland の制限**: 外部アプリへのドラッグアウトはネイティブ Wayland では動作しません（Chromium/Electron の制限、CHANGELOG 参照）。必要な場合は `npm start -- --ozone-platform=x11` で起動してください
