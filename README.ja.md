...
- ドラッグ＆ドロップを全プラットフォームでネイティブ OS ドラッグに再設計。外部アプリへ実ファイルとして渡り（Wayland を含む）、同じドラッグのアプリ内ドロップも動作
- Move on drag-out：通常のドラッグで外部アプリへ渡した後、相手側のコピーを検証（サイズ + SHA-256）してから元をゴミ箱へ。Ctrl/Option ドラッグはコピー。Customize で ON/OFF
- 修正：Wayland での起動クラッシュ（3.5 以降）、Ubuntu 24.04 以降での `npm start` のサンドボックス中断、コンテキストメニューのサブメニュー（New ▸）が表示されない問題（3.6 以降）
- Electron 28 → 44
- ローテーション付きファイルログ（`~/.config/prism-fm/logs/prism-fm.log`）、Customize に Debug log トグルと Open Log Folder。ファイル操作・D&D・ナビゲーション・エラーを記録
...
- **ドラッグ＆ドロップ**: 外部アプリへのネイティブドラッグ、外部からのドロップおよびペイン間のドラッグは移動（Ctrl キーでコピー、macOS は Option キー）。外部アプリへのドラッグアウトは Electron の制限（electron/electron#7207）により相手側では常に「コピー」になります。**Move on drag-out**（Customize ダイアログ、既定 ON）が有効なら、相手側にできたコピーを検証したうえで元ファイルをゴミ箱へ移動します。Ctrl（macOS は Option）を押しながらドラッグするとコピーのままになります
